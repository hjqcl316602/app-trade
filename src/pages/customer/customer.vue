...
export default {
  ...
  methods: {
    ...
    getClipboardImage() {
      let self = this;
      this.$refs["editable"].onpaste = function(event) {
        let value = event.clipboardData.items[0];
        if (value.kind === "file" && value.type.indexOf("image/") !== -1) {
          self.sendPicture(value.getAsFile(), "clipboard");
          return false;
        }
      };
    },
...
<template>
  <div class="app-customer">
    <Modal
      ...
      width="100%"
    >
      <img :src="chat.picture.value" alt="" class="is-preview" />
    </Modal>
