  ...
  <div class="container exchange" :class="skin">
    ...
    <div class="main">
      ...
      <div class="right">
        <div class="coin-menu">
          <div class="sc_filter">
            <span @click="changeBaseCion('ttm')" :class="{active:basecion==='ttm'}">TTM</span>
          ...
          <Table @on-current-change="gohref" highlight-row id="USDT" v-show="basecion==='usdt'" :columns="coins.columns" :data="coins.USDT"></Table>
           <Table @on-current-change="gohref" highlight-row id="TTM" v-show="basecion==='ttm'" :columns="coins.columns" :data="coins.TTM"></Table>
        ...
        .then(response => {
          ...
          if (resp.ask && resp.ask.items) {
            ...
            if (resp.ask.items.length >= this.plate.maxPostion) {
              ...
              const rows = this.plate.askRows,
                len = rows.length,
                totle = rows[this.plate.maxPostion - resp.ask.items.length] ? rows[this.plate.maxPostion - resp.ask.items.length].totalAmount :0;
              this.plate.askTotle = totle;
            }
          }
          if (resp.bid && resp.bid.items) {
            ...
            if (resp.bid.items.length < this.plate.maxPostion) {
              ...
              const rows = this.plate.bidRows,
                len = rows.length,
                totle = rows[resp.bid.items.length - 1]?rows[resp.bid.items.length - 1].totalAmount : 0 ;
