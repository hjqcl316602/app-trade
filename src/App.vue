 ...
 * @Author: your name
 * @Date: 2019-02-15 15:37:17
 * @LastEditTime: 2019-09-05 17:13:50
...
<template>
    <div :class="pageView">
                            ...
                            >
                                <Submenu name="1">
                                    ...
                                    <!-- <router-link to="/customer">
                                      <MenuItem name="nav-customer">客服</MenuItem>
                                    </router-link> -->
                                </Submenu>
                            </Menu>
                        </div>
        ...
        <app-chat v-if='!!member && chat.show'></app-chat>
    </div>
</template>
<script>
    import Vue from "vue";
    ...
    import AppChat from "./components/chat/index.js";
    ...
    export default {
        name: "app",
        data() {
            let close = localStorage.getItem("app/trade/audio/close/order/new");
            return {
        ...
        watch: {
            activeNav: function () {
                switch (this.activeNav) {
                    case "nav-exchange":
                        break;
                    default:
                        window.document.title = "国际数字加密资产交易平台";
                        break;
                }
            },
            $route(to, from) {
                ...
                /**
                 * @description:  是否隐藏客服进入按钮
                 */
                let chatHidePage = ["/customer", "/customer-order"];
                this.chat.show = !chatHidePage.includes(to.path);
            },
            exchangeSkin() {
            ...
            ["audio.close"](val) {
                if (!val) {
                    this.clearTimer("audio"); // 清除音频播放
                }
                localStorage.setItem(
                    "app/trade/audio/close/order/new",
                    val ? "close" : "open"
                );
            },
            member(val, oldVal) {
                let newId = val ? val.id : "";
                let oldId = oldVal ? oldVal.id : "";
                if (newId !== oldId) {
                    this.setTimerOrder();
                }
            }
        ...
        methods: {
             ...
             */
            getOrderNew() {
                if (!Boolean(this.member.id)) return;
                this.$http
                    .post(this.host + "/otc/order/traddingOrder")
                    .then(response => {
                        let data = response["body"];

                        if (data["code"] === 0) {
                            let buy = Number(data["data"]["buy"]);
                            let sell = Number(data["data"]["sell"]);
                            let appeal = Number(data["data"]["appeal"]);
                            ...
                            localStorage.setItem("app/order/new/log/err", Number(count) + 1);
                            //this.$Message.error("获取新订单信息提示失败！");
                        }
                    })
                    .catch(err => {
                        let count = localStorage.getItem("app/order/new/log/err");
            ...
            checkLogin() {
                this.$http.post(this.host + "/uc/check/login", {}).then(response => {
                    ...
                    if (result.code == 0) {
                        let member = Object.assign(result.data);
