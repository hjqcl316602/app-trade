        ...
        <div class="rightarea-con">
          <div class="form-group">
            ...
            <Button
              type="warning"
              @click="queryOrder"
              style="padding: 6px 30px;margin-left:10px;background-color:#f0a70a;border-color:#f0a70a"
              >{{ $t("uc.finance.record.search") }}</Button
            >
          </div>
    ...
    getAddrList() {
      ...
      this.$http
        ...
        .then(response => {
          ...
          if (resp.code == 0) {
            this.coinList = resp.data.length > 0 ? resp.data : [];
            if (this.coinType) {
    ...
    tableColumnsRecord() {
      ...
      columns.push({
        ...
        render: function(h, params) {
          let type = params.row && params.row.type;
          let str = that.recordType.filter(item => {
